...
class PyRosettaCalculator:
    ...
    def calculate_features(self, residue_number):
        ...
        try:
            if not self.pose:
                raise ValueError("PyRosetta has not loaded the PDB structure")
            if residue_number < 1 or residue_number > self.pose.total_residue():
                raise ValueError(f"residue {residue_number} out of range [1, {self.pose.total_residue()}]")
            ...
            print(f" Residue {residue_number} Feature calculation failed: {str(e)}")
    ...
    def load_custom_esm(self, protein_id):
        ...
        if os.path.exists(esm_path):
            try:
                ...
                print(f"Successfully loaded ESM feature file: {esm_path}")
                return data['features']  # 形状: [seq_len, esm_dim]
            except Exception as e:
                print(f"Failed to load ESM feature file: {esm_path}, 错误: {str(e)}")
                return None
        else:
            print(f"ESM feature file does not exist: {esm_path}")
            return None
        ...
        if self.rosetta_calc is not None:
            for i, pos in enumerate(positions):
                try:
                    ...
                    print(f"Warning: Structural feature calculation failed for residue {pos}: {str(e)}")
...
if __name__ == "__main__":
    ...
    # 打印结果摘要
    print(f"=== {ptm_type} Prediction Summary ===")
    print(f"Target amino acids: {target_aa}")
    print(f"Total number of {''.join(target_aa)} sites: {len(results_df)}")
    print(f"Number of predicted {ptm_type} sites: {len(results_df[results_df['prediction'] == 1])}")
    ...
    if len(high_prob_sites) > 0:
        print(f"\nHigh-probability sites (probability > 0.7):")
        for _, row in high_prob_sites.head(10).iterrows():
            print(f"Position {row['position']} ({row['residue']}): Predicted probability = {row['probability']:.3f}")
